...
class KeyMazeSolver(BaseSolver[SolutionState, tuple[KeyMazeNode, ...]]):
    ...
    @staticmethod
    def update_states(
        state: SolutionState, current_node: tuple[KeyMazeNode, ...], new_node: tuple[KeyMazeNode, ...]
    ) -> list[SolutionState]:
        for i in range(len(current_node)):
            new_node_i = new_node[i]
            current_node_i = current_node[i]
            if new_node_i.is_key() and current_node_i.value == new_node_i.value and current_node_i.z != new_node_i.z:
    ...
    @staticmethod
    def heuristic(node: tuple[KeyMazeNode, ...], end: (SolutionState, tuple[KeyMazeNode, ...])) -> float:
        return - sum(n.z for n in node)

    @staticmethod
    def is_destination(node: tuple[KeyMazeNode, ...], end: tuple[KeyMazeNode, ...]) -> bool:
    ...
    @staticmethod
    def evaluate_path_length(path: list[tuple[SolutionState, tuple[KeyMazeNode, ...]]]) -> int:
