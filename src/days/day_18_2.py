...
class KeyMazeSolver(BaseSolver[SolutionState, KeyMazeNode]):
    ...
    def update_state(state: SolutionState, current_node: KeyMazeNode, new_node: KeyMazeNode) -> SolutionState:
        if new_node.is_key() and current_node.value == new_node.value:
            return SolutionState(state.unlocked_keys + (new_node.value,))
        return state
    ...
    @staticmethod
    def heuristic(node: (SolutionState, KeyMazeNode), end: (SolutionState, KeyMazeNode)) -> float:
        return 0.0
...
print(solver.evaluate_path_length(solver.solve(start, destination)))
